...
import { A, Code, CodeBlock, H2, Li, Ol, P, Ul } from '@/Components'
...
export default release(
  <>
    ...
    <P>
      Previously, to cancel a visit, you would call <Code>event.preventDefault()</Code> from a <Code>start</Code> global
      event listener. You could also cancel a visit by returning <Code>false</Code> from the <Code>onStart()</Code>{' '}
      event callback. This is no longer possible with the <Code>start</Code> event. Instead, you can now do this with
      the <Code>before</Code> event:
    </P>
    <CodeBlock
      language="js"
      children={dedent`
    ...
    <P>
      The goal here is to underline the purpose of this method, which is to reload the current URL while maintaining the
      current component state (<Code>preserveState: true</Code>) as well as the current scroll position (
    ...
    <P>
      For these reasons, we have deprecated it and will be removing it in an upcoming release. Here's how to update your
      apps:
    </P>
    <CodeBlock
      language="js"
    ...
    <P>
      This release corresponds to the <Code>@inertiajs/progress</Code> version <Code>0.2.0</Code> release, which updates
      the progress library to take advantage of the improved "finish" event handling. If you're using{' '}
      <Code>@inertiajs/progress</Code>, be sure to upgrade to <Code>^0.2.0</Code> at the same time, otherwise you'll run
      into issues with the progress bar not being removed properly.
    </P>
  </>
)
