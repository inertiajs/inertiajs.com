...
In certain situations it's necessary to preserve the page component state. For example, when submitting a form, you need to preserve the your form data in the event that validation errors come back.
...
You can cancel an visit using a cancel token, which Inertia automatically generates and provides via the `onCancelToken()` callback prior to making the visit.
...
The preferred approach is to use the new [event callbacks](#event-callbacks) instead. For example, instead of using `then()`, use the `onSuccess()` callback instead.
...
Instead of using `finally()`, use the `onFinish()` callback instead.
