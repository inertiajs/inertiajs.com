...
The `onError()` callback is fired when the Inertia response contains a non empty `errors` prop. It won't fire when a request returns an error response code. If you want to intercept non 200 error codes check the [Error handling section](https://inertiajs.com/error-handling) of the docs.


```js
Inertia.post(`/users/${user.id}`, {
  onError: () => alert('Please check your fields.'),
})
```
