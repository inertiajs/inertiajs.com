...
resolveComponent: name => import(`@/Pages/${name}`).then(({ default: component }) => {
  component.layout = component.layout === undefined ? Layout : component.layout
  return component
}),
```
...
resolveComponent: name => import(`@/Pages/${name}`).then(({ default: component }) => {
  if (!name.startsWith('Public/') && component.layout === undefined) {
