...
Some events (`start`, `invalid`, `error`) support cancellation, allowing you to prevent Inertia's default behaviour. Just like native events, if only one event listener calls `event.preventDefault()` or returns `false`, the event will be cancelled.
...
Inertia.on('progress', event => {
  this.form.progress = e.detail.progress.percentage
})
```
