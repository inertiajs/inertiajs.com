...
When making a non-GET Inertia request, via `<inertia-link>` or manually, be sure to always respond with a proper Inertia
response.
...
Note, when redirecting after a `PUT`, `PATCH` or `DELETE` request you must use a `303` response code, otherwise the
subsequent request will not be treated as a `GET` request. A `303` redirect is the same as a `302` except that the
follow-up request is explicitly changed to a `GET` request.
