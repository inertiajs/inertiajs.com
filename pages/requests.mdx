...
`preserveState` supports a callback if you want to decide whether or not to preserve local component state based on the _response_. For example, if you want to preserve form state after there were validation errors and display fresh data after a user successfully submitted the form.
  ...
  examples={[
    {
      ...
      code: dedent`
        ...
        this.$inertia.post(url, data, {
          replace: false,
          preserveState: page => Object.values(page.errors).length > 0,
          preserveScroll: false,
          only: [],
        })
      `,
...
`preserveScroll` supports a callback if you want to decide whether or not to reset the scroll position based on the _response_.
  ...
  examples={[
    {
      ...
      code: dedent`
        ...
        this.$inertia.post(url, data, {
          replace: false,
          preserveState: true,
          preserveScroll: page => Object.values(page.errors).length > 0,
          only: [],
        })
      `,
    },
