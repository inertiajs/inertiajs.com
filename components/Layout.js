          ...
          <header className="py-12 relative flex items-center justify-between">
            <Link href="/">
              <a className="md:flex md:items-end">
                ...
                <svg className="block fill-current text-white mt-2 md:mt-0 md:ml-4" css={{ height: '8px' }} viewBox="0 0 328.3 16">
            ...
            <div ref={mobileNav} className="md:hidden relative z-10">
              <button className="block focus:outline-none" type="button">
                <svg className="block fill-current text-white w-6 h-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                  <path d="M0 3h20v2H0V3zm0 6h20v2H0V9zm0 6h20v2H0v-2z"></path>
                </svg>
              </button>
              {showMobileNav && <Nav className="absolute top-0 right-0 -mt-4 p-8 bg-white shadow-xl rounded z-50 whitespace-no-wrap" />}
            </div>
              ...
              <div className="w-full lg:w-1/2 mt-4">
                ...
                <p className="mt-8 text-lg lg:text-xl leading-relaxed max-w-md pr-2">
                  ...
                  <strong className="font-bold" css={{ color: '#92eee2' }}>
                    quickly build modern <span className="whitespace-no-wrap">single-page</span> React, Vue.js and Svelte apps
                  </strong>{' '}
                ...
                <ul>
                  {meta.links.map((link, index) => (
                    <li className="mt-4" key={index}>
                      <Link href={link.url}>
                        <a className="hover:underline font-medium text-gray-700">{link.name}</a>
                      </Link>
                    </li>
                  ))}
                </ul>
